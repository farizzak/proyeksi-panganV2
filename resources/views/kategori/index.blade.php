...
@section('content')
  <div class="container mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-800 mb-6">
      <h2 class="text-2xl font-semibold text-gray-800 dark:text-white">Kategori</h2>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
      ...
      <div class="md:col-span-5">
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm p-6">
          <form id="myform" method="POST" action="{{ route('kategori.store') }}" class="space-y-6">
            @csrf
            <input type="hidden" name="_method" id="method" value="post">

            <h3 id="formtitle" class="text-lg font-semibold text-gray-800 dark:text-gray-100">Add Kategori</h3>

            <div>
              <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Kategori</label>
              <input
                type="text"
                id="name"
                name="name"
  ...
  <div id="deleteModal" class="fixed inset-0 z-50 items-center justify-center bg-black/50 hidden">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg w-full max-w-md p-6">
      <div class="text-center space-y-4">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">Are you sure you want to delete this Kategori?</p>
      </div>
...
@push('scripts')
  ...
  $(document).ready(function () {
    ...
    $form.on('submit', function (e) {
      ...
      const url = isCreate ? "{{ route('kategori.store') }}" : `${baseUrl}/${$form.data('edit-id')}`;
    ...
    function resetForm() {
      ...
      $form.attr('action', "{{ route('kategori.store') }}")[0].reset();
      $('#name').blur();
    }
    ...
    $('#dataTable').on('click', '.delete-btn', function () {
      ...
      Swal.fire({
        title: 'Are you sure?',
        text: `Kategori "${$('<div>').html(name).text()}" akan dihapus!`,
        icon: 'warning',
